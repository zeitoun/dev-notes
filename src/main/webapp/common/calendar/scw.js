...
//******************************************************************************
    ...
    var scwDateOutputFormatArb  = 'YYYY/MM/DD'; // e.g. 'MMM-DD-YYYY' for the US
    ...
    document.writeln(
        '<style type="text/css">'                                       +
            ...
            '/* Calendar background */'                                 +
            'table.scw                  {background-color:  #DDDDDD;}'  +
            '/* Drag Handle */'                                         +
            'td.scwDrag                 {background-color:  #9999CC;'   +
                                        'color:             #CCCCFF;}'  +
            '/* Week number heading */'                                 +
            'td.scwWeekNumberHead       {color:             #EFEFEF;}'  +
            '/* Week day headings */'                                   +
            ...
            'td.scwCellsExMonth         {background-color:  #CCCCCC;'   +
                                        'color:             #666666;}'  +
            '/* Today selector */'                                      +
            'td.scwFoot                 {background-color:  #FF0000;'   +
                                        'color:             #FFDDDD;}'  +
            ...
            '/* Active cells */'                                        +
            'td.scwCells:hover,'                                        +
            'td.scwCellsHover           {background-color:  #EFEFEF;'   +
                                        'cursor:            pointer;'   +
                                        'color:             #000000;}'  +
            '/* Day matching the input date */'                         +
            'td.scwInputDate:hover,'                                    +
            ...
            '/* Weekend cells */'                                       +
            'td.scwCellsWeekend:hover,'                                 +
            'td.scwCellsWeekendHover    {background-color:  #DFDFDF;'   +
                                        'cursor:            pointer;'   +
                                        'color:             #000000;}'  +
            '/* Day outside the current month */'                       +
            'td.scwCellsExMonth:hover,'                                 +
            'td.scwCellsExMonthHover    {background-color:  #EEEEEE;'   +
                                        'cursor:            pointer;'   +
                                        'color:             #000000;}'  +
            '/* Today selector */'                                      +
            'td.scwFoot:hover,'                                         +
            'td.scwFootHover            {background-color:  #EE0000;'   +
                                        'color:             #FFFFFF;'   +
                                        'cursor:            pointer;'   +
                                        'font-weight:       bold;}'     +
            '/* Disabled cells */'                                      +
...
// ****************************************************************************
    ...
    function scwShow(scwEle,scwSourceEvent)
        {
    var inputDate = scwEle.value;
//        		alert(inputDate);
//        	if(window.top.language=='Arb'){
//        		var arbDate = new Date(inputDate);
//        		var arbDay = arbDate.getDay();
//        		var arbMonth = arbDate.getMonth();
//        		var arbYear = arbDate.getFullYear();
//        		inputDate = arbYear + '/' + arbMonth + '/' + arbD;
//        		alert(arbYear);
//        		alert(inputDate);
//        	}
         ...
         if (inputDate) {
         		scwDateValue = inputDate.replace(/^\s+/,'').replace(/\s+$/,'');
         }
            ...
            {scwSelYears.options.selectedIndex=Math.floor(scwMonthSum/12);
             ...
             function scwSetOutput(scwOutputDate)
                {if (typeof scwTargetEle.value == 'undefined')
                 ...
                 else {scwTargetEle.value = scwOutputDate.scwFormat(scwDateOutputFormat);//scwOutputDate.scwFormat(window.top.language=='Eng'?scwDateOutputFormat:scwDateOutputFormatArb);
                 }
                 scwHide();
                };
