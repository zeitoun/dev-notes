...
//added by Houssein Dhayne to know when we press clear all , on the date range calendar
//to clear only the selected field and not the two fields
//because of problem when there is hidden value that contain a date that we need to check if the selected date
//is after or before then when the user clear all the  selected field must be only cleared and not the two fields
var selected =""
...
gsBottom=(NN4?"":"<DIV class='BottomDiv'>")+"<A class='BottomAnchor' href='javascript:void(0)' onclick='if(this.blur)this.blur();fClearSelected();return false;' onmouseover='return true;'>Clear</A>"+(NN4?"":"</DIV>");


var _startc,_endc;
function fStartPop(startc,endc,fct) {
  _startc=startc;
  _endc=endc;
  var sd=fParseInput(endc.value); 
  if (!sd) sd=gEnd;
  fPopCalendar(startc, [gBegin,sd,sd]);
  if(fct == null) fct = function() {};
  afterSel = fct;
  selected ="START";
}

function fEndPop(startc,endc, fct) {
  _startc=startc;
  _endc=endc;
